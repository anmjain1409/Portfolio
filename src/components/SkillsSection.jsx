import { useState } from "react";
import { cn } from "@/lib/utils";

const skills = [
  // Frontend
  { name: "HTML5", level: 90, category: "frontend" },
  { name: "CSS3", level: 85, category: "frontend" },
  { name: "JavaScript (ES6+)", level: 80, category: "frontend" },
  { name: "React.js", level: 80, category: "frontend" },

  // Backend
  { name: "Python", level: 90, category: "backend" },
  { name: "Java", level: 75, category: "backend" },
  { name: "Spring Boot", level: 75, category: "backend" },
  { name: "Flask", level: 80, category: "backend" },
  { name: "Node.js", level: 70, category: "backend" },
  { name: "Express.js", level: 70, category: "backend" },

  // Databases (Separate Section 🔥)
  { name: "MySQL", level: 80, category: "databases" },
  { name: "PostgreSQL", level: 75, category: "databases" },
  { name: "MongoDB", level: 75, category: "databases" },
  { name: "Neo4j", level: 75, category: "databases" },

  // AI / ML
  { name: "Machine Learning", level: 80, category: "aiml" },
  { name: "NLP", level: 80, category: "aiml" },
  { name: "Hugging Face", level: 75, category: "aiml" },
  { name: "OpenCV", level: 70, category: "aiml" },

  // Tools
  { name: "Git", level: 85, category: "tools" },
  { name: "GitHub", level: 85, category: "tools" },
  { name: "Docker", level: 70, category: "tools" },
  { name: "Postman", level: 80, category: "tools" },
  { name: "VS Code", level: 90, category: "tools" },

  // Concepts
  { name: "Data Structures & Algorithms", level: 75, category: "concepts" },
  { name: "REST APIs", level: 85, category: "concepts" },
  { name: "JWT Authentication", level: 80, category: "concepts" },
];

const categories = [
  "all",
  "frontend",
  "backend",
  "databases",
  "aiml",
  "tools",
  "concepts",
];

export const SkillsSection = () => {
  const [activeCategory, setActiveCategory] = useState("all");

  const filteredSkills = skills.filter(
    (skill) => activeCategory === "all" || skill.category === activeCategory
  );

  return (
    <section id="skills" className="py-24 px-4 relative bg-secondary/30">
      <div className="container mx-auto max-w-5xl">
        <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center">
          My <span className="text-primary">Skills</span>
        </h2>

        {/* Category Buttons */}
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {categories.map((category, key) => (
            <button
              key={key}
              onClick={() => setActiveCategory(category)}
              className={cn(
                "px-5 py-2 rounded-full transition-colors duration-300 capitalize",
                activeCategory === category
                  ? "bg-primary text-primary-foreground"
                  : "bg-secondary/70 text-foreground hover:bg-secondary"
              )}
            >
              {category === "aiml"
                ? "AI/ML"
                : category === "databases"
                  ? "Databases"
                  : category}
            </button>
          ))}
        </div>

        {/* Skills Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredSkills.map((skill, key) => (
            <div
              key={key}
              className="bg-card p-6 rounded-lg shadow-sm card-hover"
            >
              <div className="text-left mb-4">
                <h3 className="font-semibold text-lg">{skill.name}</h3>
              </div>

              <div className="w-full bg-secondary/50 h-2 rounded-full overflow-hidden">
                <div
                  className="bg-primary h-2 rounded-full origin-left animate-[grow_1.5s_ease-out]"
                  style={{ width: skill.level + "%" }}
                />
              </div>

              <div className="text-right mt-1">
                <span className="text-sm text-muted-foreground">
                  {skill.level}%
                </span>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};